...
<template>
      ...
      <Card class="h-full">
        <template #content>
          <div class="flex flex-row">
            ...
            <h2 class="text-2xl font-bold leading-7 text-gray-900 m-6 sm:truncate sm:text-3xl sm:tracking-tight">{{
              props.title }}</h2>
          </div>
          <slot />
        </template>
  ...
  <transition name="slide">
    <div class="fixed inset-y-0 left-0 bg-white w-64 p-4 z-50 transform transition-transform duration-300 lg:hidden"
